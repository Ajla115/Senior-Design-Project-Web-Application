...
const DeleteAdminModal = ({ isOpen, onClose, customerId, refetch }) => {
  const client = new QueryClient();
  const [isDeleted, setIsDeleted] = useState(false);

  const mutation = useMutation({
    mutationFn: async () => {
      const response = await UserService.deleteAdmin(customerId);
      return response;
    },
    onSuccess: (data) => {
      setIsDeleted(true);
      ...
      <Fade in={isOpen}>
        <ModalContent sx={style}>
          ...
          <p id="transition-modal-description" className="modal-description">
            Are you sure you want to delete admin with ID: {customerId}?
          </p>
